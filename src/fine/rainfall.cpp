...
RainfallRecorder::RainfallRecorder(z3::context& context, std::ostream& output,
                                   std::string run)
    : context_(context), output_(output),
      run_(run.empty() ? fresh_run() : std::move(run)) {}
...
std::string RainfallRecorder::string_array(
...
RainfallField RainfallRecorder::string_field(std::string name,
                                              std::string_view value) {
    return {std::move(name), quote(value)};
}

RainfallField RainfallRecorder::number_field(std::string name,
                                              std::size_t value) {
...
void RainfallRecorder::record(
    std::string_view kind, std::string_view operation,
    ...
    output_ << "{\"schema\":\"fine.rainfall.v2\",\"run\":" << quote(run_)
            ...
            << ",\"event_id\":\"event:" << current << "\",\"sequence\":"
            << current << ",\"kind\":" << quote(kind)
            << ",\"operation\":" << quote(operation)
            << ",\"within\":" << string_array(within)
            << ",\"producer\":{\"component\":" << quote(producer)
            << ",\"coverage\":" << quote(coverage) << "},\"data\":{";
...
std::string RainfallRecorder::term(z3::expr const& expression,
    ...
    identity << "{\"run\":" << quote(run_)
             << ",\"recorder\":\"recorder:0\",\"manager\":\"manager:0\""
             << ",\"handle\":" << handle
             << ",\"ast_id_at_observation\":"
             << Z3_get_ast_id(context_, expression) << '}';
    record("object", "term.declare", {}, "fine.ast-registry",
           ...
           {string_field("id", reference),
            raw_field("identity", identity.str()),
            string_field("representation", representation),
            string_field("text", expression.to_string())});
    return reference;
}
