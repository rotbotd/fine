...
function normalized_integer() -> Bool { -0 == 0 }
        ...
        StageEvaluation known_identity = infer_stage(executable, "through_identity", {stage_boolean(true)});
        StageEvaluation runtime_identity =
            infer_stage(executable, "through_identity", {stage_runtime({syntax::ValueType::Kind::boolean, {}})});
        StageEvaluation dead =
            infer_stage(executable, "dead_arm", {stage_runtime({syntax::ValueType::Kind::boolean, {}})});
        StageEvaluation live =
            infer_stage(executable, "live_join", {stage_runtime({syntax::ValueType::Kind::enumeration, "Flag"})});
        StageEvaluation boxed = infer_stage(executable, "boxed", {});
        StageEvaluation normalized = infer_stage(executable, "normalized_integer", {});
...
function right(flag: Flag, value: Bool) -> Bool { left(flag, value) }
        ...
        StageEvaluation mutual_stage = infer_stage(mutual, "left",
