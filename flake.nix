    ...
    in {
      ...
      default = pkgs.stdenv.mkDerivation {
        ...
        installCheckPhase = ''
          ...
          grep -F "changed-constant-caller-miss: true" <<<"$stage_analysis"
          grep -F "same-export-leaf-miss: true" <<<"$stage_analysis"
          grep -F "same-export-caller-hit: true" <<<"$stage_analysis"
          grep -F "nonconstant-transform-caller-miss: true" <<<"$stage_analysis"
          grep -F "nonconstant-transform-caller-result: comptime(false)" <<<"$stage_analysis"
          grep -F "known-identity: comptime(true)" <<<"$stage_analysis"
          grep -F "runtime-identity: runtime" <<<"$stage_analysis"
          grep -F "dead-arm-result: comptime(true)" <<<"$stage_analysis"
          grep -F "dead-arm-edges: 1" <<<"$stage_analysis"
          grep -F "live-join-result: runtime" <<<"$stage_analysis"
          grep -F "live-join-edges: 2" <<<"$stage_analysis"
          grep -F "constructor-field-result: comptime(true)" <<<"$stage_analysis"
          grep -F "known-tag-runtime-payload: comptime(true)" <<<"$stage_analysis"
          grep -F "known-tag-edges: 1" <<<"$stage_analysis"
          grep -F "cross-call-known-tag: comptime(true)" <<<"$stage_analysis"
          grep -F "cross-call-known-tag-edges: 1" <<<"$stage_analysis"
          grep -F "capture-avoiding-result: comptime(true)" <<<"$stage_analysis"
          grep -F "capture-avoiding-edges: 3" <<<"$stage_analysis"
          grep -F "strict-argument-result: comptime(true)" <<<"$stage_analysis"
