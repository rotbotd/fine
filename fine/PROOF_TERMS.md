...
For one ground index beyond the finite table, Fine has a narrower exact rule. It
collects every family reachable through indexed constructor premises and gives
each one a private Horn relation. Constructor values are universally quantified,
identity proof demands become equality guards, and proof fields become premises
of the corresponding family relation. Spacer then queries the closed index
against the joint least relation system. Only `unsat` strengthens the evidence
cover to false. Source function calls are excluded before rule installation
because Z3 can hang while installing ground facts about recursive functions. A
reachable index, an unknown result, a timeout, a source call, or a symbolic index
falls back to the conservative constructor cover. This is why a self-supported
constructor cannot make `Even(succ(zero))`, `Zero(-1)`, or an outer wrapper around
such evidence inhabited, while the paired reachable-index controls still reject
empty matches. The Horn relations are analysis artifacts and never become Fine
proof values.
